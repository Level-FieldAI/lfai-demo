  ...
  return <>
    <div className={cn("flex items-center justify-center w-full h-full", {
      ...
      <div className='relative w-full h-full'>
        <Button variant='outline' onClick={handleToggleMode} className='absolute top-2 right-2 z-20 gap-2' size='sm'>
        ...
        {remoteParticipantIds.length > 0 ? (
          <Video
            ...
            className={cn({
              // Mobile: 9:16 aspect ratio, centered
              'w-full h-full max-w-full aspect-[9/16] mx-auto': isMobile && mode === 'full',
              // Desktop: 16:9 aspect ratio (aspect-video)
              'w-full h-full aspect-video': !isMobile && mode === 'full',
              // Minimal mode
          ...
          <div className={cn('relative flex items-center justify-center w-full bg-slate-100 rounded-md', {
            // Mobile: 9:16 aspect ratio, centered
            'h-full max-w-full aspect-[9/16] mx-auto': isMobile && mode === 'full',
            // Desktop: 16:9 aspect ratio
            'h-full aspect-video': !isMobile && mode === 'full',
            // Minimal mode
            'max-w-xs aspect-[9/16]': mode === 'minimal',
          })}>
            <p className={cn('text-black text-center px-4', {
              'text-lg': isMobile,
              'text-2xl': !isMobile,
        ...
        {localSessionId && (
          <Video
            ...
            className={cn('absolute z-10 border-2 border-white shadow-lg rounded-md overflow-hidden', {
              // Mobile: Smaller user video, positioned to avoid overlap
              'top-4 right-4 w-20 h-28 aspect-[9/16]': isMobile && mode === 'full',
              // Desktop: Standard positioning
              'bottom-6 right-6 w-1/4 min-w-[120px] max-w-[180px] aspect-video': !isMobile && mode === 'full',
              // Minimal mode - responsive positioning
              'top-2 right-2 w-16 h-20 aspect-[9/16]': isMobile && mode === 'minimal',
